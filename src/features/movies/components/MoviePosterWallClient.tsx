/**
 * File: `src/features/movies/components/MoviePosterWallClient.tsx`
 * Purpose:
 * - 电影海报墙的“交互层”组件（Client Component）。
 *
 * 负责的能力：
 * 1) 当页面滚动到电影区域时：整块区域与海报 grid 做入场动画（whileInView + stagger）
 * 2) hover 海报：海报有霓虹/酸性风格的浮起、微旋转、发光边框效果
 * 3) 点击海报：弹出详情卡片（modal），展示电影的详细信息
 *   - 液态玻璃风：半透明渐变 + 强 backdrop-blur + 高光描边
 *   - 打开/关闭：详情卡片自点击海报中心缩放进出
 *   - 桌面左栏海报：`object-cover` 铺满固定高度侧栏；右栏 `.movie-detail-scroll` 样式化滚动条
 * 4) 网格：卡片尺寸向摄影区对齐，使用更舒展的 2～5 列响应式布局
 *
 * 设计取舍：
 * - 动画实现用 `framer-motion`，避免纯 CSS 在复杂弹层场景里触发不稳定
 * - Modal 使用可访问性友好的结构：遮罩层 + card + 图标关闭按钮 + Esc 关闭
 * - 首页电影预览与 `/movies` 列表共用一套卡片尺寸语言，减少视觉割裂
 *
 * 注意：
...
function buildCardVariants(): Variants {
  return {
    hidden: (origin: ModalOrigin | null) => {
      ...
      return { opacity: 0.45, scale: s, x: dx, y: dy };
    },
    visible: {
      opacity: 1,
      scale: 1,
      x: 0,
      y: 0,
      transition: { duration: 0.38, ease: [0.16, 1, 0.32, 1] },
    },
...
export default function MoviePosterWallClient({
  ...
  return (
    <section className={sectionClassName ?? "mt-12"}>
      ...
      {/* 进场动画：当滚动进入视口时触发一次 */}
      <motion.div
        className="mt-6 grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5"
          ...
          <>
            ...
            <div className="pointer-events-none fixed inset-0 z-[51] flex items-center justify-center px-4 py-8 sm:px-7">
              ...
              >
                ...
                />
                <div
                  className="pointer-events-none absolute inset-px rounded-[inherit] ring-1 ring-white/15 ring-inset"
                  aria-hidden
                />
                    ...
                    <div className="movie-detail-scroll min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-5 py-5 pr-3 sm:px-6 sm:py-6 sm:pr-4">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <button
                          ...
                          onClick={() => setActiveId(null)}
                          aria-label="关闭电影详情"
                          className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/12 bg-white/5 text-zinc-100 transition hover:border-[#39ff14]/45 hover:bg-white/10 hover:text-[#39ff14]"
                        >
                          <svg
                            viewBox="0 0 24 24"
                            aria-hidden="true"
                      ...
                      <dl className="mt-4 grid gap-2 text-sm text-zinc-300 sm:grid-cols-2">
                        {activeMovie.directors?.length ? (
                          <div className="sm:col-span-2">
                            <dt className="text-xs font-semibold tracking-wider text-[#39ff14]/85 uppercase">
                              导演 / Director
                            </dt>
                      ...
                      <div className="mt-5">
                        <div className="text-xs font-semibold tracking-widest text-[#39ff14]/90 uppercase">
                          Tagline
                        </div>
                      ...
                      <div className="mt-5">
                        <div className="text-xs font-semibold tracking-widest text-[#00ffff]/80 uppercase">
                          影片简介 / Overview
                        </div>
                      ...
                      {activeMovie.castTop?.length ? (
                        <div className="mt-5">
                          <div className="text-xs font-semibold tracking-widest text-[#bf00ff]/85 uppercase">
                            主要演员 / Cast
                          </div>
