/**
 * File: `src/features/movies/routes/MoviesIndexRoute.tsx`
 * Purpose:
 ...
 *
 * 设计要点：
 * - 分页逻辑在 `src/features/movies/lib/movies.ts`，这里仅做“路由层组合”
 * - 这样未来换数据源（例如从 CMS/API）不会牵连 UI 组件
 */

import Link from "next/link";
import MoviePosterWallClient from "@/features/movies/components/MoviePosterWallClient";
import MoviesBackgroundBackdrop from "@/features/movies/components/MoviesBackgroundBackdrop";
import { getMoviesCatalog, getMoviesPage } from "@/features/movies/lib/movies";

function toInt(value: string | string[] | undefined): number | null {
  if (!value) return null;
  const nextValue = Array.isArray(value) ? value[0] : value;
  const numberValue = Number.parseInt(nextValue, 10);
        ...
        <div className="mt-10 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {Array.from({ length: data.totalPages }).map((_, index) => {
              ...
              const isActive = paginationNumber === data.page;

              return (
                <Link
                  key={paginationNumber}
                  href={`/movies?page=${paginationNumber}`}
                  className={[
